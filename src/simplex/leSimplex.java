...
public class leSimplex {
    ...
    public static void main(String[] args) {
        ...
        double[][] Restrica = {{5, 0, 1, 9}, {0, 1, 1, 3}, {1, 2, -1, 24}, {7, 4, 1, 4}};
        double[] func = {75, 10};

        try {
        //quantidade de variaveis de decisão, quantidade de restrições, matriz, função obejtiva
            montaMatriz(2, 3, Restrica, func);
    ...
    public static double[][] montaMatriz(int NumVarDecisao, int numRestricao, double[][] MatrizRestricao,
            double[] FObjetiva) {
        ...
        for (int i = 0; i < numRestricao; i++) {
            tabela[i][coluna - 1] = MatrizRestricao[i][NumVarDecisao + 1]; // preencher ultima coluna com os valores da última coluna da Matriz Restrição

            for (int j = 0; j < coluna - 1; j++) {
                if (j == NumVarDecisao) {
                    tabela[i][j + i] = MatrizRestricao[i][j]; // Insere os coeficientes das variaveis de folgas
                } else if (j < numRestricao) {
                    tabela[i][j] = MatrizRestricao[i][j]; // Insere os coeficientes das variaveis de decisão
                }
            }
        }
    ...
    public static void CalcularSimplex() {
        ...
        System.out.print("Simplex Terminado\n");
        for (int i = 0; i < linha; i++) {
            for (int j = 0; j < coluna; j++) {
                System.out.print(tabela[i][j] + "   ||   ");
            }
